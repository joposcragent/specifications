...
2. Если тело валидно — ставит **новое** независимое фоновое задание сбора для данного запроса (отдельный экземпляр браузера) и **немедленно** возвращает `HTTP 200`. Несколько подряд или параллельных валидных запросов порождают несколько одновременных заданий; вызывающая сторона должна учитывать нагрузку на CPU и RAM контейнера или хоста.
   1. При возникновении любого исключения в ходе запуска джоба возвращает `HTTP 500` с текстом исключения в теле ответа.
3. Для каждого поискового запроса из `{searchQueries.list}`:
   ...
   3. Для каждой страницы:
      ...
      8. Для каждой новой вакансии:
         ...
         3. Сохраняет вакансию через `job-postings-crud`: `uid`, `title`, `company`, `url`, `content`, `publicationDate`:
            ...
            3. Если ответ `HTTP 409 Conflict` — **не** считается ошибкой: в лог пишется предупреждение уровня WARN, исключение не пробрасывается, обход переходит к следующей вакансии; такая вакансия **не** учитывается как успешно сохранённая (в частности, не попадает в `pageVacancyUids` для событий оркестратора и не увеличивает счётчик сохранённых записей).
4. При возникновении любого исключения — пропускает вакансию и продолжает (skip-and-continue). Ответ `HTTP 409` на `POST …/job-postings/{jobPostingUuid}` исключением не считается (см. последний подпункт в блоке «Сохраняет вакансию через job-postings-crud» выше).
...
2. **После каждой успешно обработанной страницы** (после шага 3.3 для данной страницы, до перехода к следующей странице) crawler отправляет в оркестратор `POST …/queue-broker/events/collection-query-progress` с телом JSON по схеме `CollectionQueryProgressEvent`:
...
sequenceDiagram
    ...
    alt невалидное тело
        ...
        Note over Crawler,HH: асинхронный фоновый процесс по list из запроса (каждый POST — своё задание)
