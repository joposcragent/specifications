...
- [`async-job.job-posting-create-result`] — итог обработки `begin`: успешная вставка или ошибка сохранения (в том числе откат транзакции / исключение при вставке).
...
| ------------------- | ---------- | ------------- |
| 📌`{key}` | Ключ записи Kafka (`record.key`) | Если непустой, передаётся в исходящие заголовки как `key`; иначе для исходящего `key` используется строка `{payload}.jobUuid` |
...
4. Валидирует и маппит `{payload}` в модель сохранения (обязательны непустые поля: `jobUuid`, `searchQueryUuid`, `uid`, `title`, `url`, `company`, `content`, `publicationDate`; идентификатор вакансии задаётся полем `jobPostingUuid` или, при его отсутствии, полем `entityUuid`).
   1. `publicationDate` приводится к строке даты: допускается `date-time` (берётся календарная дата в локальной зоне смещения) или дата в формате `YYYY-MM-DD`.
   2. При любой ошибке разбора пишет предупреждение в лог с причиной и завершает работу **без** публикации результата в Kafka.
...
| `schemaVersion` | `1.0` |
| `payload.jobUuid` | UUID задания из входящего `payload.jobUuid` |
