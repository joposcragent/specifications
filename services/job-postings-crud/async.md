...
4. Валидирует и маппит `{payload}` в модель сохранения.
   ...
   4. `publicationDate` приводится к строке даты: допускается `date-time` (берётся календарная дата в локальной зоне смещения) или дата в формате `YYYY-MM-DD`.
   5. При ошибке обязательных полей (отсутствие, пусто, неверный UUID или дата) пишет в лог **error** и публикует результат со статусом `FAILED` и текстом причины в `result.message`, **если** удалось определить `jobUuid` задания; иначе только **error** в лог **без** публикации в Kafka.
