...
2. Для каждого элемента массива поле **`previousRun`** заполняется из
   **оперативного кэша** планировщика для соответствующего `job_type` (как у
   `GET /settings`), либо `null`, если в кэше нет записи для типа.
3. В массив включаются строки для всех значений перечисления, включая те,
   для которых запись в таблице `scheduler` отсутствует.
4. Полученный объект `SchedulerSettingsList` с полем `list` возвращается с
   кодом HTTP 200.
5. При любом не перехваченном исключении возвращает HTTP 500 с текстом
   ошибки.

## Принудительный немедленный запуск джоба

`POST /execute?jobType={jobType}`
...
1. Валидирует входные параметры и при ошибке возвращает HTTP 400 с
   описанием `IncorrectRequest`.
2. Асинхронно запускает выполнение соответствующего джоба и при успешном
   запуске возвращает HTTP 200 без тела
3. При любом не перехваченном исключении возвращает HTTP 500 с текстом
   ошибки.
