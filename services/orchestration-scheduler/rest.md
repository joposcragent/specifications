...
1. На основании данных в таблице `joposcragent.orchestration.scheduler` и
   строит массив объектов `SchedulerSettingsItem`.
2. В массив включаются строки для всех значений перечисления, включая те,
   для которых запись в таблице `scheduler` отсутствует.
3. Полученный массив возвращает с кодом HTTP 200
