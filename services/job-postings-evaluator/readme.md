...
**Алгоритм:** [п.1](#1-загрузка-порога-и-эталонного-вектора), [п.2](#2-отбор-строк-в-postings) с целевым набором `{jobPostingUuid}`; при ошибках и пустом отборе — [таблица](#поведение-при-ошибках-и-пустом-отборе) (асинхронно). Затем [п.3](#3-оценка-каждой-отобранной-строки), [п.4](#4-запись-в-бд). Исключения — по [таблице](#поведение-при-ошибках-и-пустом-отборе).
...
**Алгоритм:** тот же смысл, что у [`POST /evaluate/sync/list`](#синхронный-запуск-оценки): [п.1](#1-загрузка-порога-и-эталонного-вектора), [п.2](#2-отбор-строк-в-postings) с целевым набором из `{uuids.list}`, далее [п.3](#3-оценка-каждой-отобранной-строки) и [п.4](#4-запись-в-бд); при ошибках и пустом отборе — [таблица](#поведение-при-ошибках-и-пустом-отборе) (синхронно).

Возвращает **HTTP 200** и тело `JobPostingsUidsEvaluatedList`: для вакансий, попавших в отбор на п.2, их `uuid` и новые значения `evaluationStatus` после выполнения [п.3–4](#3-оценка-каждой-отобранной-строки). При необработанном исключении — **HTTP 500** с текстом исключения в теле ответа.
